...
export function getUserColumns(onPreview: (url: string, isImage: boolean) => void): ColumnDef<User>[] {
    return [
        ...
        //             setLoading(true);
        //             try {
        //                 // await updateUserStatus(id, value ? "ACTIVE" : "INACTIVE");
        //                 setChecked(value);
        //                 toast.success(`Utilisateur ${id} ${value ? "activé" : "désactivé"}.`);
        //             } catch (error) {
        //                 setChecked(!value);
        ...
        },
        {
            ...
            header: "CNI",
            cell: ({ row }) => {
                const carte = row.original.carte;
                return carte ? (
                    <Button
                        variant="link"
                        onClick={() => onPreview(carte, isImage(carte))}
        ...
        },
        {
            ...
            header: "PERMIS",
            cell: ({ row }) => {
                const permis = row.original.permis;
                return permis ? (
                    <Button
                        variant="link"
                        onClick={() => onPreview(permis, isImage(permis))}
                        className="p-0 h-auto text-blue-600 hover:underline"
                    >
                        Voir le permis
                    </Button>
